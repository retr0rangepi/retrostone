...
#joystick =====================
	...
	joystick_LR = int(f3raw.read())*float(f3scale.read())
	joystick_UD = int(f4raw.read())*float(f4scale.read())
	if (joystick_LR > 1350):
		joystick_LR = 1350
	if (joystick_UD > 1350):
		joystick_UD = 1350
...
#Check and apply joystick states	
	if (joystick_LR > (VREF/2 + DZONE)) or (joystick_LR < (VREF/2 - DZONE)):
		gamepad.emit(uinput.ABS_X, joystick_LR ) #- 50 - 100 * (joystick_LR < VREF/2 - DZONE) + 100 * (joystick_LR > VREF/2 + DZONE)
	...
	if (joystick_UD > (VREF/2 + DZONE)) or (joystick_UD < (VREF/2 - DZONE)):
		gamepad.emit(uinput.ABS_Y, joystick_UD ) #+ 50 - 100 * (joystick_UD < VREF/2 - DZONE) + 100 * (joystick_UD > VREF/2 + DZONE)
	else:#Center the sticks if within deadzone
		gamepad.emit(uinput.ABS_Y, VREF/2)
